...
## Project Overview
Myco is a decentralized orchestration system. It provides a CLI for project init, daemon management (gossip, API), service deployment (Nix, Systemd), and peer management.
...
1.  **Fast Feedback (Unit Tests):**
    ```bash
    zig build test && zig build test-sim && zig build test-utils
    ```
...
3.  **Simulation Tests (Specific):**
    `make sim-50-realworld` (for gossip/orchestration changes).
4. **Document what was done and why in the log/ folder:**
