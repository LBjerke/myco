...
fn handleDaemonCommand(frozen_alloc: *FrozenAllocator) !void {
    try runDaemon(frozen_alloc);
}

fn handleStatusCommand() !void {
    try queryDaemon("GET /metrics HTTP/1.0\r\n\r\n");
}

fn handlePubkeyCommand() !void {
...
fn handlePeerCommand(args_it: *@TypeOf(std.process.args())) !void {
...
fn handleDeployCommand() !void {
    try myco.cli.deploy.run();
}

fn dispatchCommand(command: []const u8, args_it: *@TypeOf(std.process.args()), frozen_alloc: *FrozenAllocator) !void {
    if (std.mem.eql(u8, command, "init")) {
        try handleInitCommand();
    } else if (std.mem.eql(u8, command, "daemon")) {
        try handleDaemonCommand(frozen_alloc);
    } else if (std.mem.eql(u8, command, "status")) {
        try handleStatusCommand();
    } else if (std.mem.eql(u8, command, "pubkey")) {
        try handlePubkeyCommand();
    } else if (std.mem.eql(u8, command, "peer")) {
        try handlePeerCommand(args_it);
    } else if (std.mem.eql(u8, command, "deploy")) {
        try handleDeployCommand();
    } else {
        printUsage();
    }
}
...
pub fn main() !void {
    ...
    try dispatchCommand(command, &args_it, &frozen_alloc);
}
